...
class CouchDBStore:
    ...
    def get_id(self, parameter=None):
        ...
        response = requests.get(self.url + '_uuids/').json()
    ...
    def put(self, db_name, data, document_id=None):
        try:
            ...
            return {"ok": "false", "message": "Data provided is not a dict"}
        except Exception as e:
            # TODO: Change this to catch specific exceptions
            return {"ok": "false", "message": "{} raised at put()".format(e)}
    ...
    def get(self, db_name, coordinate=None, document_id=None):
        try:
            ...
            # TODO: Change this to catch specific exceptions
            return {"ok": "false", "message": "{} raised at get()".format(e)}
...
class PersistentStore(CouchDBStore):
    ...
    def store(self, db_name, list_of_data, document_id=None):
        ...
        for data in list_of_data:
            try:
                document = self.put(db_name=db_name, data=data,
                    document_id=document_id)
                self.put(db_name="coordinates", data={"_id": document["id"],
                        "coordinate": data["coordinate"], "calculated": "false"})
            except Exception as e:
                # TODO: Change this to catch specific exceptions
                return {"ok": "false", "message": "{} raised at store()".format(e)}
        return document
    ...
    def retrieve(self, db_name, coordinate=None, document_id=None, n=10,
        processed=None):
        if document_id:
            document = self.get(db_name=db_name, document_id=document_id)
        elif coordinate:
            document = self.get(db_name=db_name, coordinate=coordinate)
        elif processed is not None:
            document = self.get_n(db_name=db_name, n=n, processed=processed)
        else:
            document = self.get(db_name=db_name)
            # TODO: Change this to catch specific exceptions  
        return document
